...
public class Payload extends ControlPoint {
  ...
  private void tickParticles(long tick) {
    if (!definition.getDisplayFilter().query(new MatchQuery(null, match)).isAllowed()) return;
  ...
  private void updateWool() {
    Competitor display = getDisplayTeam();
    DyeColor color = display != null ? DyeColor.getByColor(display.getFullColor()) : DyeColor.WHITE;
