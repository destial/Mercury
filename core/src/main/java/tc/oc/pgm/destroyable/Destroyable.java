...
public class Destroyable extends TouchableGoal<DestroyableFactory>
    ...
    if (this.isDestroyed()) {
      ...
      this.match.callEvent(
          new GoalCompleteEvent(
              this.getMatch(), this, this.getOwner(), isOwnerBreak(), this.getContributions()));
    }
  ...
  public String testBlockChange(
    ...
    if (deltaHealth < 0) {
      // Damage
      if (player != null && !canComplete(player.getParty())) {
  ...
  public boolean isOwnerBreak() {
    return this.definition.ownerBreaks;
  }
