...
public class TeamMatchModule implements MatchModule, Listener, JoinHandler {
  public static boolean USE_ELO_BALANCING = true;

  static class NeedMorePlayers implements UnreadyReason {
  ...
  public final Cache<UUID, Team> lastLeft =
  ...
  @Override
  public void unload() {
    MatchModule.super.unload();
    lastLeft.invalidateAll();
  }
  ...
  public boolean canSwitchTeams(MatchPlayer joining) {
    return canChooseTeam(joining) || canBuyTeam(joining);
  }
  ...
  public boolean canBuyTeam(MatchPlayer joining) {
    JSONObject data = joining.getPlayerData().getData();
  ...
  public boolean areTeamsEvenAfterJoin(@Nullable MatchPlayer joining, @Nullable Team newTeam) {
    boolean useElo = PGM.get().getConfiguration().shouldQueueElo() && USE_ELO_BALANCING;
    if (!useElo) {
      ...
      int first = elos.get(0);
      int last = elos.get(elos.size() - 1);
      double averageElo =
          (double)
                  getParticipatingTeams().stream()
                      .flatMapToInt(t -> IntStream.of((int) t.getElo()))
                      .sum()
              / getParticipatingTeams().size();
      return last - first <= averageElo;
    }
    return true;
  }
  ...
  public Team getFullestTeam() {
    boolean useElo = PGM.get().getConfiguration().shouldQueueElo() && USE_ELO_BALANCING;
    if (useElo) {
      List<Team> sorted = new ArrayList<>(getParticipatingTeams());
      sorted.sort(Comparator.comparingDouble(Team::getElo));
  ...
  public TeamJoinResult getEmptiestJoinableTeam(MatchPlayer joining, boolean priorityKick) {
    ...
    boolean useElo = PGM.get().getConfiguration().shouldQueueElo() && USE_ELO_BALANCING;
    if (useElo) {
      shuffledTeams.sort((a, b) -> (int) (a.getElo() - b.getElo()));
    } else {
      ...
      if (team != joining.getParty()) {
        ...
        if (result.isSuccess()) {
          ...
          if (!useElo) {
            ...
            if (bestResult == null || result.getTeam().getElo() < bestResult.getTeam().getElo()) {
              bestResult = result;
            }
          }
        }
  ...
  private JoinResult queryJoin(MatchPlayer joining, @Nullable Team chosenTeam, boolean queued) {
    ...
    if (chosenTeam == null) {
      ...
      if (canSwitchTeams(joining) || chosenTeam != lastTeam) {
        ...
        if (!canChooseTeam(joining) && !canBuyTeam(joining)) {
  ...
  public boolean join(MatchPlayer joining, @Nullable Competitor chosenParty, JoinResult result) {
    if (result instanceof TeamJoinResult) {
      ...
      if (!canChooseTeam(joining) && chosenParty != null && chosenParty != lastTeam) {
        JSONObject data = joining.getPlayerData().getData();
        int left = data.has("team_selections") ? data.getInt("team_selections") : 0;
        if (left <= 0) {
          joining.sendWarning(TranslatableComponent.of("join.err.noChoice"));
        ...
        joining.getPlayerData().setData("team_selections", --left);
        ChatDispatcher.broadcastAdminChatMessage(
            joining
                .getName()
                .append(TextComponent.of(" used their pass to join ", TextColor.YELLOW))
                .append(chosenParty.getName()),
            joining.getMatch());
      }
  ...
  public void balanceTeams() {
    ...
    if (!PGM.get().getConfiguration().shouldQueueElo()) return;
  ...
  public void onLeave(PlayerLeaveMatchEvent e) {
    if (!(e.getPlayer().getParty() instanceof Team)) return;
    lastLeft.put(e.getPlayer().getId(), (Team) e.getPlayer().getParty());
  }

  @EventHandler
  public void onJoin(PlayerPartyChangeEvent e) {
    if (e.getNewParty() instanceof Team) {
      if (lastLeft.getIfPresent(e.getPlayer().getId()) != null) {
        lastLeft.invalidate(e.getPlayer().getId());
      }
    }
  }
