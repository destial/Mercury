...
public class MapPoll {
  ...
  private final Map<Balance, Set<UUID>> eloBalancing = new HashMap<>();
  ...
  MapPoll(Match match, Map<MapInfo, Double> mapScores, Set<MapInfo> overrides, int voteSize) {
    ...
    for (Balance balance : Balance.values()) {
      eloBalancing.put(balance, new HashSet<>());
    }
  ...
  public void announceWinner(MatchPlayer viewer, MapInfo winner) {
    ...
    if (PGM.get().getConfiguration().shouldQueueElo()) {
      Balance balance = getMostVotedBalance();
      viewer.sendMessage(
          TextComponent.of(
              "Next match will use " + balance.name() + " balancing.", TextColor.GREEN));
    }
  ...
  public void sendBook(MatchPlayer viewer, boolean forceOpen) {
    if (viewer.isLegacy()) {
      ...
      if (PGM.get().getConfiguration().shouldQueueElo()) {
        viewer.sendMessage(TextComponent.newline());
        for (Balance balance : Balance.values())
          viewer.sendMessage(getBalanceVoteComponent(viewer, balance));
      }
      return;
    }
    ...
    if (PGM.get().getConfiguration().shouldQueueElo()) {
      content.append(TextComponent.newline());
      for (Balance balance : Balance.values())
        content.append(TextComponent.newline().append(getBalanceVoteComponent(viewer, balance)));
    }
  ...
  private Component getBalanceVoteComponent(MatchPlayer viewer, Balance balance) {
    boolean voted = eloBalancing.get(balance).contains(viewer.getId());
    ...
    return TextComponent.builder()
        ...
        .append(
            TextComponent.of(" ").decoration(TextDecoration.BOLD, !voted)) // Fix 1px symbol diff
        .append(balance.name() + " Balancing", TextColor.GOLD, TextDecoration.BOLD)
        .clickEvent(ClickEvent.runCommand("/votebalance -o " + balance.name()))
        .build();
  }
  ...
  public boolean toggleBalance(Balance balance, UUID player) throws CommandException {
    Set<UUID> votes = this.eloBalancing.get(balance);
    if (votes == null) throw new CommandException(balance.name() + " is not an option in the poll");
  ...
  private Balance getMostVotedBalance() {
    return eloBalancing.entrySet().stream()
        .max(Comparator.comparingInt(e -> countVotes(e.getValue())))
        .map(Map.Entry::getKey)
        .orElse(null);
  }
  ...
  MapInfo finishVote() {
    ...
    if (PGM.get().getConfiguration().shouldQueueElo()) {
      Balance balance = getMostVotedBalance();
      TeamMatchModule.USE_ELO_BALANCING = balance == Balance.ELO;
    } else {
      TeamMatchModule.USE_ELO_BALANCING = false;
    }
    if (match != null) {
      match.getPlayers().forEach(player -> announceWinner(player, picked));
      match.callEvent(new VoteCompleteEvent(picked, votes.get(picked).size()));
    }
