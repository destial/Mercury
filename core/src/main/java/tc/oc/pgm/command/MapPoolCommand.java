...
public final class MapPoolCommand {
  ...
  @Command(aliases = "votebalance", desc = "Vote for the balance option", usage = "ELO or DEFAULT")
  public void voteBalance(
      MatchPlayer player,
      CommandSender sender,
    ...
    if (poll != null) {
      MapPoll.Balance balance = MapPoll.Balance.valueOf(arg);
      boolean voteResult = poll.toggleBalance(balance, ((Player) sender).getUniqueId());
      Component voteAction =
          TranslatableComponent.of(
              voteResult ? "vote.for" : "vote.abstain",
              voteResult ? TextColor.GREEN : TextColor.RED,
              TextComponent.of(balance.name()));
