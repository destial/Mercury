...
public final class PGMConfig implements Config {
  ...
  private final List<String> sussyClients;
  ...
  private final Duration defaultRespawnDelay;
  ...
  PGMConfig(FileConfiguration config, File dataFolder) throws TextException {
    ...
    this.defaultRespawnDelay =
        parseDuration(config.getString("gameplay.default-respawn-delay", "2s"));
    ...
    this.eloJoin = parseBoolean(config.getString("join.elo", "false"));
    ...
    File eloFile = new File(dataFolder, "elo.yml");
    if (!eloFile.exists()) {
      PGM.get().saveResource("elo.yml", false);
    }
    this.eloConfig = new EloConfig(YamlConfiguration.loadConfiguration(eloFile));

    this.sussyClients =
        config.contains("sus-clients") ? config.getStringList("sus-clients") : new ArrayList<>();
  }
  ...
  @Override
  public void reloadEloConfig() {
    eloConfig = new EloConfig(YamlConfiguration.loadConfiguration(new File(dataFolder, "elo.yml")));
  }
  ...
  @Override
  public Duration getDefaultRespawnDelay() {
  ...
  @Override
  public List<Config.Group> getGroups() {
    return groups;
  }

  @Override
  public List<String> getSussyClients() {
    return sussyClients;
  }
    ...
    private static Permission getPermission(
      ...
      for (String permission : config.getStringList(realm)) {
        if (permission.startsWith("-")) {
          permissions.put(permission.substring(1).trim(), false);
        } else if (permission.startsWith("+")) {
          permissions.put(permission.substring(1).trim(), true);
        } else {
          permissions.put(permission.trim(), true);
        }
      }
